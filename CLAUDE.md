...
- 以下の操作は作業開始時に必ず行ってください
  - **作業開始時**: 必ず `develop-mvvm` ブランチから専用ブランチを作成する（feat-<機能名>、fix-<修正内容>等）
  - **mainブランチ・developブランチでの直接作業は絶対禁止**: いかなる変更もmainブランチやdevelopブランチに直接コミットしない
- 以下を必ず作業終了時に実行してください。
  1. 作業内容をコミット
  2. リモートブランチにpush (`git push -u origin <ブランチ名>`)
  3. PR作成 (ghコマンドでPR作成)
     - **重要: PRのベースブランチは必ず `develop-mvvm` を指定すること**（`--base develop-mvvm`）
     - `develop` ブランチをベースにしてはいけません
  - @ai-rules/COMMIT_AND_PR_GUIDELINES.mdにガイドラインを記述しています。上記の作業時には必ず確認して必ず内容に従って作業を行ってください。

### 主要機能
- **目標管理**: 学習目標の作成・編集・削除・達成状況の追跡
- **タイマー機能**: 学習時間の計測と記録（カウントダウン/カウントアップ）※ポモドーロは未実装（将来実装予定）
...
- **要件が曖昧な場合は、ターゲットユーザーを考慮した提案を行いつつヒアリングすること**
  - まず **@docs/TARGET_USERS.md** を参照してターゲットユーザーを理解する
  - ターゲットユーザーに適した選択肢を提案しつつ、ユーザーに確認する
  - 単に選択肢を並べるのではなく、推奨案とその理由を説明する
- Issueを作成する前に以下を確認：
  - 実装内容が明確か
  - 受け入れ条件が定義されているか
  - 不明点がないか
- 不明点がある場合は、ユーザーに質問して要件を明確化してから作成する
- **新機能・UI提案時は、ターゲットユーザーに適切かを確認する**
  - プライマリ: 小学校高学年〜高校生（10〜18歳）の受験勉強・定期試験
  - セカンダリ: 大学生（19〜25歳）の資格試験・就活準備
  - 求めていること: シンプルなUI、学習時間の可視化、モチベーション維持
  - 避けるべき: 複雑な機能、SNS的な要素、過度なゲーミフィケーション、小中学生が理解できない機能
  - 競合（目標達成タイマー）との差別化を意識する

---

## 7. `spec-complete` ラベル付与基準

### 付与条件
以下のすべてが記載されているIssueに `spec-complete` ラベルを付与する：

1. **概要** - 何を実装するかが明確
2. **機能仕様** - UI構成、動作フローが記載
3. **テストケース** - ID付きテーブル形式で記載
4. **完了条件** - 以下を含むチェックリスト

### 完了条件に必須の項目
- [ ] 機能の実装
- [ ] ユニットテスト作成・通過
- [ ] Lintチェック通過（`flutter analyze`）
- [ ] 全テスト通過（`flutter test`）
- [ ] IPAビルド確認（`flutter build ios --release --no-codesign`）
...
## 15. PRマージ手順（「PR #XXをマージして」と言われた場合）
...
| ファイル | 内容 | 参照タイミング |
|---------|------|---------------|
| `TARGET_USERS.md` | ターゲットユーザー定義・競合分析 | Issue作成時・新機能提案時 |
