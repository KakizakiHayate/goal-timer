...
class GoalDetailScreenV2 extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<GoalDetailScreenV2> createState() => _GoalDetailScreenV2State();
}

class _GoalDetailScreenV2State extends ConsumerState<GoalDetailScreenV2>
  ...
  Widget _buildProgressOverviewCard() {
    final progress = widget.goal.getProgressRate();
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildProgressDetail(
                      label: '目標時間',
                      value: '${widget.goal.targetMinutes}分',
  ...
  Widget _buildAvoidanceCard() {
    if (widget.goal.avoidMessage.isEmpty) return const SizedBox.shrink();
  ...
  void _startTimer() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => TimerScreenV2(goalId: widget.goal.id),
      ),
    );
  }

  void _showEditModal() async {
    final result = await GoalEditModalV2.show(context, widget.goal);
