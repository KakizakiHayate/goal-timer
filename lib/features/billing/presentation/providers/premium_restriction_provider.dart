...
final premiumRestrictionServiceProvider = Provider<PremiumRestrictionService>((ref) {
  final repository = ref.watch(billingRepositoryProvider);
  return PremiumRestrictionService(repository);
});

/// 目標作成可能チェックプロバイダー
final canCreateGoalProvider = FutureProvider.family<bool, int>((ref, currentGoalCount) async {
...
/// 残りの目標数プロバイダー
final remainingGoalsProvider = FutureProvider.family<int, int>((ref, currentGoalCount) async {
  final service = ref.watch(premiumRestrictionServiceProvider);
  return await service.getRemainingGoals(currentGoalCount);
});

/// 目標制限メッセージプロバイダー
final goalLimitMessageProvider = FutureProvider.family<String, int>((ref, currentGoalCount) async {
  final service = ref.watch(premiumRestrictionServiceProvider);
