...
class UpdateGoalUseCase {
  ...
  Future<GoalsModel> call({
    ...
    try {
      AppLogger.instance.i('🔧 [UpdateGoalUseCase] 目標の更新を開始します: ${originalGoal.title}');
      AppLogger.instance.i('🔧 [UpdateGoalUseCase] 更新前の目標ID: ${originalGoal.id}');
      ...
      if (updatedTitle.length < minTitleLength) {
        AppLogger.instance.e('❌ [UpdateGoalUseCase] バリデーションエラー: タイトルは$minTitleLength文字以上である必要があります');
        throw ArgumentError('タイトルは$minTitleLength文字以上である必要があります');
      }
      if (updatedAvoidMessage.isEmpty) {
        AppLogger.instance.e('❌ [UpdateGoalUseCase] バリデーションエラー: ネガティブ回避メッセージは必須です');
        throw ArgumentError('ネガティブ回避メッセージは必須です');
      }
      if (updatedAvoidMessage.length < minAvoidMessageLength) {
        AppLogger.instance.e('❌ [UpdateGoalUseCase] バリデーションエラー: ネガティブ回避メッセージは$minAvoidMessageLength文字以上である必要があります');
      ...
      if (updatedTargetMinutes < minTargetMinutes || updatedTargetMinutes > maxTargetMinutes) {
        AppLogger.instance.e('❌ [UpdateGoalUseCase] バリデーションエラー: 目標時間は$minTargetMinutes分から$maxTargetMinutes分（23時間59分）の範囲で設定してください');
      ...
      AppLogger.instance.i('🔧 [UpdateGoalUseCase] モデルタイトル: ${updatedGoal.title}');
      AppLogger.instance.i('🔧 [UpdateGoalUseCase] 目標時間: ${updatedGoal.targetMinutes}分');

      // リポジトリ経由でデータベースを更新
      AppLogger.instance.i('🚀 [UpdateGoalUseCase] リポジトリのupdateGoal()を呼び出します...');
      final resultGoal = await _repository.updateGoal(updatedGoal);
      
      AppLogger.instance.i('✅ [UpdateGoalUseCase] リポジトリのupdateGoal()が完了しました');
      AppLogger.instance.i('📊 [UpdateGoalUseCase] 結果: ${resultGoal.title} (ID: ${resultGoal.id})');
      AppLogger.instance.i('📊 [UpdateGoalUseCase] 目標時間: ${resultGoal.targetMinutes}分');
